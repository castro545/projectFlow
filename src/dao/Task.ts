...
export class TaskDAO implements TaskInterface {
  ...
  async createTask(project_id: number, colaborator: string, task_name: string, task_description: string, owner_code: number): Promise<number> {

    const query = `select create_task(${project_id}, 1, $1, $2, $3, now()::timestamp, now()::timestamp, ${owner_code})`;
    const values = [colaborator, task_name, task_description];
    try {
  ...
  async fetchCountTaskByProject(user_code: number, project_code: number | null): Promise<CountTaskInfo[]> {
    const query = `
      SELECT
        ...
        SUM(CASE WHEN project_user.user_code = ${user_code} THEN 1 ELSE 0 END) AS created_by_me
      FROM project_user
          INNER JOIN projects ON projects.project_id = project_user.project_code
      LEFT JOIN tasks ON tasks.project_code = project_user.project_code
      WHERE projects.project_id = $1
      AND project_user.user_code = ${user_code}
  ...
  async fetchCountTaskByUser(user_code: number): Promise<CountTaskInfo[]> {
    const query = `
      SELECT
        SUM(CASE WHEN status_code IN(1, 2, 4) THEN 1 ELSE 0 END) AS pending_tasks,
        SUM(CASE WHEN status_code IN(3, 5) THEN 1 ELSE 0 END) AS completed_tasks,
        SUM(CASE WHEN project_user.user_code = $1 THEN 1 ELSE 0 END) AS created_by_me
      FROM project_user
          INNER JOIN projects ON projects.project_id = project_user.project_code
      LEFT JOIN tasks ON tasks.project_code = project_user.project_code
      WHERE project_user.user_code = $2
      AND tasks.is_active = '1'
          AND projects.is_active = '1'
      AND project_user.is_active = '1'
    `;
